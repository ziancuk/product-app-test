    ...
    <table id="example" class="display text-center py-4" style="width:100%">
        ...
        @foreach($user as $i)
            <tr>
                ...
                <td>
                    @if($i->status == 0)
                    <button type="button" class="px-2 w-full h-8 text-white rounded-full" style="background-color: red;">Tidak Aktif</button type="button">
                    @else
                    <button type="button" class="px-2 w-full h-8 text-white rounded-full" style="background-color: #15803d;">Aktif</button type="button">
                    @endif
                </td>
                <td class="flex justify-center items-center">
                    ...
                    <div class="px-1">
                        <svg class="cursor-pointer w-6 h-6 text-white rounded-full p-1" data-modal-target="small-modal" data-modal-toggle="small-modal" data-name="{{ $i->name }}" data-id="{{ $i->id }}" type="button" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 20" style="background-color: red;">
        ...
        <div class="relative bg-white w-full max-w-md mx-auto rounded-lg shadow-lg">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="px-6 py-6 lg:px-8">
                    <h3 class="text-center mb-4 text-lg font-medium text-gray-900 dark:text-white">Tambah User</h3>
                    <form action="create-user" method="POST">
                        @csrf
                        <div class="mb-2">
                            <label for="name" class="block mb-2 text-sm text-gray-400">Nama Lengkap</label>
                            <input type="text" name="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full" placeholder="Masukkan Nama Lengkap" required>
                        </div> 
                        <div class="mb-2">
                            <label for="email" class="block mb-2 text-sm text-gray-400">Email</label>
                            <input type="email" name="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full" placeholder="Masukkan Email" required>
                            @error('email')
...
$(document).ready( function () {
    $('#example').DataTable({
        ...
        $("#formDelete").attr("action", `delete-user/${id}?`);
    });
</script>
@endsection
